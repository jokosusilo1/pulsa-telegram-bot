...
bot.onText(/📊 CEK HARGA REAL/, async (msg) => {
    ...
    if (prices.success && prices.data) {
        // Filter hanya pulsa dan ambil 10 produk pertama
        const pulseProducts = prices.data
            .filter(p => p.category === 'pulsa' && p.brand && p.price)
            .slice(0, 10);
        
        if (pulseProducts.length > 0) {
            let message = '📋 **DAFTAR HARGA PULSA REAL**\n\n';
            
            pulseProducts.forEach(product => {
                message += `📱 ${product.product_name}\n💵 Rp ${product.price.toLocaleString()}\n🏷️ ${product.brand}\n\n`;
            });
            
            message += '_Data real-time dari Digiflazz_';
